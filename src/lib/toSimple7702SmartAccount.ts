import { Account, Address, BaseError, Chain, decodeFunctionData, EIP1193Provider, encodeFunctionData, LocalAccount, OneOf, Prettify, PrivateKeyAccount, Transport, TypedData, TypedDataDefinition, WalletClient } from "viem"
import { entryPoint08Abi, entryPoint08Address, getUserOperationTypedData, SmartAccount, SmartAccountImplementation } from "viem/account-abstraction"
...
export type Simple7702SmartAccountImplementation = SmartAccountImplementation<
  ...
  '0.8',
  { abi: typeof abi; owner: OneOf<EthereumProvider, WalletClient<Transport, Chain | undefined, Account>> },
  true
>
...
export async function toSimple7702SmartAccount(
  ...
  return toSmartAccount({
    ...
    async signMessage(parameters) {
      ...
      return await localOwner.signMessage({ message })
    },

    async signTypedData(parameters) {
      const { domain, types, primaryType, message } =
        parameters as TypedDataDefinition<TypedData, string>
      return await localOwner.signTypedData({
        domain,
        message,
        primaryType,
        types,
